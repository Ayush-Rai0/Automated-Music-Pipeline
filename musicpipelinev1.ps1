...
Function Organize-AudioFiles {
    param ([string]$SourcePath, [string]$DestinationPath, [bool]$UseOnline, [string]$StepName)
    ...
    foreach ($file in $files) {
        ...
        $onlineData = $null
        if ($UseOnline) { $onlineData = Search-OnlineMetadata -SearchTerm $cleanBaseName }
        ...
        # THE FIX: Explicitly build the arguments list with exact quotes around paths
        $argsList = @()
        $argsList += "-y"
...
Write-Host "`nDo you want to use the internet (iTunes database) to fix missing metadata and download Cover Art?" -ForegroundColor Cyan
$useNet = Read-Host "Choice (Y/N)"
$isOnline = ($useNet -match '^y')
...
if ($isOnline) {
    ...
    while (-not $connected) {
        ...
        try {
            ...
            Write-Host "[!] Internet unavailable or iTunes is down." -ForegroundColor Red
            $retryChoice = Read-Host "Press [R] Retry, [O] Offline Mode, or [E] Exit (R/O/E)"
            if ($retryChoice -match '^o') { $isOnline = $false; Write-Host "Switching to Offline Mode...`n" -ForegroundColor Cyan; break } 
            elseif ($retryChoice -match '^e') { exit }
        }
    }
}
...
# ----------------------------------------------------------------------------
Write-Host "`n[Step 1/6] Separating Audio and Video files..." -ForegroundColor Magenta
# ----------------------------------------------------------------------------
...
# ----------------------------------------------------------------------------
Organize-AudioFiles -SourcePath $audioDir -DestinationPath $finalLibraryDir -UseOnline $isOnline -StepName "Step 3: Tagging Originals"
...
# ----------------------------------------------------------------------------
Organize-AudioFiles -SourcePath $extractedAudioDir -DestinationPath $finalLibraryDir -UseOnline $isOnline -StepName "Step 5: Tagging Extracted"
