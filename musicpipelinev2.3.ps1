...
# Version 2.3 - Optimized (Parser-Safe Format)
# ============================================================================
...
Function Organize-AudioFiles {
    ...
    foreach ($file in ($files)) {
        ...
        $baseName = ($baseName -replace '\((?![^\)]*\b(?:19|20)\d{2}\b)[^\)]*\)', '') -replace '\[(?![^\]]*\b(?:19|20)\d{2}\b)[^\]]*\]', ''
        $baseName = ($baseName -replace '(?i)official( music)? video', '') -replace '(?i)(official )?lyrics?', ''
        ...
        if (($null) -ne ($onlineData) -and ($onlineData.ArtworkUrl)) {$coverPath = Join-Path ($env:TEMP) "$([guid]::NewGuid()).jpg"
            try {
                Invoke-WebRequest -Uri ($onlineData.ArtworkUrl) -OutFile ($coverPath) -ErrorAction SilentlyContinue | Out-Null$argsList += "-i", "`"$coverPath`"", "-map", "0:a", "-map", "1:v", "-c:a", "copy", "-c:v", "mjpeg", "-disposition:v", "attached_pic"
...
Function Remove-DuplicatesInteractive {
    ...
    foreach ($group in ($lengthGroups)) {
        ...
        $partialHashes = foreach ($file in ($group.Group)) {
            try {
                $stream = [System.IO.File]::OpenRead($file.FullName);$buffer = New-Object byte[] 1048576
        ...
        $partialGroups = ($partialHashes | Group-Object -Property PartialHash | Where-Object {$_.Count -gt 1 })
        
        # Tier 3: Full Hash on Partial Matches only
        foreach ($pGroup in ($partialGroups)) {$fullGroups = ($pGroup.Group.File | Get-FileHash -Algorithm MD5 | Group-Object -Property Hash | Where-Object {$_.Count -gt 1 })
...
$totalSep = ($allFiles.Count); $sepCount = 0

foreach ($file in ($allFiles)) {
    $sepCount++; $percent = [math]::Round(($sepCount / $totalSep) * 100)
